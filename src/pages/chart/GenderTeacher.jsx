...
export default function GenderTeacher() {
  const [chartData, setChartData] = useState([]);

  useEffect(() => {
    axios
      .get(
        "http://west.edu.mn:3000/api/v1/statistics/teachers?groupField1=gender"
      )
      .then((res) => {
        const transformed = res.data?.teachers?.reduce(
    ...
    <Card className="h-60">
      <CardHeader>
        <CardTitle className="uppercase">
          Нийт Багш ажилтны тоо: <span>{chartData[0]?.sum}</span>
        </CardTitle>
